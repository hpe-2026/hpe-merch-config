...
| 2026-07-18 | Update Project Documentation | Added Project Overview to READMEs detailing on-premise microservices architecture, OSS usage, and evaluation. |
| 2026-07-19 | Fix Node.js CommonJS Import Error | Fixed a CrashLoopBackOff in `python-service` (actually a Node.js process) by converting named exports for `@opentelemetry/resources` and `@opentelemetry/semantic-conventions` to default imports in `tracing.js`. |
| 2026-07-19 | Fix Python `pkg_resources` Import Error | Resolved a `ModuleNotFoundError: No module named 'pkg_resources'` in the actual `python-service` by explicitly adding `setuptools` to `requirements.txt`. Python 3.12 environments no longer include it by default. |
| 2026-07-19 | Patch `setuptools` Vulnerabilities | Upgraded `setuptools` in `python-service/requirements.txt` from `69.5.1` to `78.1.1` to resolve Trivy scan vulnerabilities `CVE-2024-6345` (Remote code execution) and `CVE-2025-47273` (Path Traversal). |
